...
1. ~~**中置信度累积逻辑**：连续2轮相同状态才转移，可能导致频繁切换情绪时反应迟钝~~ ✅ 已修复（v0.2.0）
2. ~~**衰减过于激进**：3轮无信号即衰减，长对话中可能过早重置~~ ✅ 已修复（v0.2.0）
3. ~~**快照截断**：只保留最近5条信号/历史，长对话中早期信号会丢失~~ ✅ 已修复（v0.2.0）

## 🔧 开发计划

- [x] P1：调整衰减机制（3轮→5轮，渐进衰减）✅ 已完成
- [x] P1：扩大快照保留范围（5条→20条）✅ 已完成
- [x] P2：添加assistant消息分析（辅助信号）✅ 已完成
- [x] P2：优化中置信度累积逻辑 ✅ 已完成
- [x] P2：添加长对话测试（50+轮）✅ 已完成

## 📝 更新日志

### v0.2.0 (2026-06-05)
**P1 修复**：
- ✅ 调整衰减机制：3轮→5轮，改为渐进衰减（高唤醒→中等唤醒→neutral）
- ✅ 扩大快照保留范围：5条→20条

**P2 优化**：
- ✅ 添加assistant消息分析：检测否定、质疑、轻视、安慰、鼓励等模式
- ✅ 优化中置信度累积逻辑：
  - 时间窗口：3轮→5轮
  - 阈值：2+→3+（相同极性）
  - 考虑情绪极性：相同极性信号可累积
